...
async function main() {
  ...
  const ownerSignup = await storage.createCompanyWithAdmin({
    companyName: "Tenant Smoke GmbH",
    userId: "tenant-owner-user",
    firstName: "Owner",
    lastName: "Admin",
    phone: "+49 89 123456",
  });

  assert.equal(ownerSignup.employee.companyId, ownerSignup.company.id);
  assert.equal(
    (await storage.getEmployeeByUserId("tenant-owner-user"))?.id,
    ownerSignup.employee.id,
  );
  assert.equal(
    (await storage.getCompanyByUserId("tenant-owner-user"))?.id,
    ownerSignup.company.id,
  );

  await assert.rejects(
    () =>
      storage.createCompanyWithAdmin({
        companyName: "Duplicate Tenant GmbH",
        userId: "tenant-owner-user",
        firstName: "Duplicate",
        lastName: "Owner",
    }),
  ...
  const inviteOwner = await storage.createCompanyWithAdmin({
    companyName: "Invite Flow GmbH",
    userId: "invite-owner-user",
    firstName: "Invite",
    lastName: "Owner",
  });

  const createdInvitation = await storage.createCompanyInvitation({
    companyId: inviteOwner.company.id,
    invitedByUserId: inviteOwner.employee.userId,
  ...
  assert.equal(
    (await storage.getActiveCompanyInvitationByToken(createdInvitation.token))?.id,
    createdInvitation.invitation.id,
  );

  await assert.rejects(
    () =>
      storage.acceptCompanyInvitation({
  ...
  const acceptedInvitation = await storage.acceptCompanyInvitation({
    token: createdInvitation.token,
    userId: "worker-user",
    userEmail: "worker@example.com",
  });

  assert.equal(acceptedInvitation.company.id, inviteOwner.company.id);
  assert.equal(acceptedInvitation.employee.userId, "worker-user");
  assert.equal(
    (await storage.getActiveCompanyInvitationByToken(createdInvitation.token)),
    undefined,
  );
