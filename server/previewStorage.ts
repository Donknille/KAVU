...
function buildPreviewData() {
  ...
  const jobs: Job[] = [
    ...
    },
    {
      ...
      description: "Stoerung am Wechselrichter pruefen.",
    ...
    },
    {
      ...
      description: "Fehlermeldung pruefen und Sensor neu einmessen.",
    ...
    },
    {
      ...
      description: "Armaturen pruefen und zwei Ventile tauschen.",
    ...
    },
    {
      ...
      title: "PV Sichtpruefung Flachdach",
    ...
    },
    {
      ...
      description: "Zwei Wohnungen pruefen und Heizkreis entlueften.",
  ...
  const assignments: Assignment[] = [
    {
      ...
      sortOrder: 0,
      note: "Module zuerst pruefen, dann Speicher anklemmen.",
      status: "on_site",
...
export class PreviewStorage {
  ...
  async deleteAssignment(id: string) {
    ...
    this.data.photos = this.data.photos.filter((photo) => photo.assignmentId !== id);
  ...
  async getPhotosByJob(companyId: string, jobId: string) {
    return sortByCreatedDesc(
      this.data.photos.filter(
        (photo) => photo.companyId === companyId && photo.jobId === jobId,
      ),
    );
  }

  async getPhotosByAssignment(companyId: string, assignmentId: string) {
    return sortByCreatedDesc(
      this.data.photos.filter(
        (photo) => photo.companyId === companyId && photo.assignmentId === assignmentId,
      ),
    );
  }

  async createPhoto(data: InsertPhoto) {
    const job = await this.getJobForCompany(data.companyId, data.jobId);
    const employee = await this.getEmployeeForCompany(data.companyId, data.employeeId);
