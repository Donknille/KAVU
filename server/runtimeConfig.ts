...
function resolveUploadProvider(): Exclude<UploadProvider, "preview"> {
  const configured = normalizeEnv(process.env.UPLOAD_PROVIDER);
  if (configured === "replit" || configured === "local") {
    return configured;
  }
...
export const UPLOAD_PROVIDER: UploadProvider = PREVIEW_MODE
  ? "preview"
...
export const LOCAL_UPLOADS_DIR = process.env.LOCAL_UPLOADS_DIR || "uploads";
...
export function getRuntimeConfigIssues(): RuntimeConfigIssue[] {
  ...
  if (UPLOAD_PROVIDER === "local" && !LOCAL_UPLOADS_DIR) {
    issues.push({
      field: "LOCAL_UPLOADS_DIR",
      message: "Local upload directory must be configured when UPLOAD_PROVIDER=local.",
      level: "error",
    });
  }
